...
function OpenShop(config, isPedMenu, shopType)
    local function startCustomization()
        client.startPlayerCustomization(function(appearance)
            if appearance then
    ...
    -- pedmenu: abrir al instante, sin round-trip de dinero
    if isPedMenu then
        startCustomization()
        return
    end
...
local function useOxPedMenu()
    return Config.PedMenuType == "menu"
end

local function toOxMenuOptions(contextOptions)
    local options = {}
...
local function registerOxLibMenu(id, title, contextOptions, parentId)
    lib.registerMenu({
        id = id,
        title = title,
        position = "top-right",
        options = toOxMenuOptions(contextOptions),
        onClose = function(keyPressed)
            if parentId and keyPressed then
                lib.showMenu(parentId)
            end
        end
    }, function(selected, _, args)
        if not selected or not args then
            return
        end
        if args.menu then
            lib.showMenu(args.menu)
        elseif args.event then
            TriggerEvent(args.event, args.eventArgs)
        end
    end)
end

local function setOxLibMenuOptions(id, contextOptions)
    lib.setMenuOptions(id, toOxMenuOptions(contextOptions))
end

local function RegisterChangeOutfitMenu(id, parent, outfits, mType, asOx)
    ...
    if asOx then
        registerOxLibMenu(id, changeOutfitMenu.title, changeOutfitMenu.options, parent)
    else
        lib.registerContext(changeOutfitMenu)
    end
end

local function RegisterUpdateOutfitMenu(id, parent, outfits, asOx)
    ...
    if asOx then
        registerOxLibMenu(id, updateOutfitMenu.title, updateOutfitMenu.options, parent)
    else
        lib.registerContext(updateOutfitMenu)
    end
end

local function RegisterGenerateOutfitCodeMenu(id, parent, outfits, asOx)
    ...
    if asOx then
        registerOxLibMenu(id, generateOutfitCodeMenu.title, generateOutfitCodeMenu.options, parent)
    else
        lib.registerContext(generateOutfitCodeMenu)
    end
end

local function RegisterDeleteOutfitMenu(id, parent, outfits, deleteEvent, asOx)
    ...
    if asOx then
        registerOxLibMenu(id, deleteOutfitMenu.title, deleteOutfitMenu.options, parent)
    else
...
local function RegisterWorkOutfitsListMenu(id, parent, menuData, asOx)
    local menu = {
        id = id,
        menu = parent,
    ...
    if asOx then
        registerOxLibMenu(id, menu.title, menu.options, parent)
    else
...
function OpenMenu(isPedMenu, menuType, menuData)
    local asOx = isPedMenu and useOxPedMenu()
    ...
    local function registerOutfitMenus(outfits)
        RegisterChangeOutfitMenu(changeOutfitMenuID, mainMenuID, outfits, nil, asOx)
        RegisterUpdateOutfitMenu(updateOutfitMenuID, mainMenuID, outfits, asOx)
        RegisterDeleteOutfitMenu(deleteOutfitMenuID, mainMenuID, outfits, "illenium-appearance:client:deleteOutfit", asOx)
        RegisterGenerateOutfitCodeMenu(generateOutfitCodeMenuID, mainMenuID, outfits, asOx)
    end

    -- Mostrar menú al instante; outfits se cargan en paralelo
    registerOutfitMenus({})
    ...
    elseif menuType == "job-outfit" then
        ...
        RegisterWorkOutfitsListMenu(workOutfitsMenuID, mainMenuID, menuData, asOx)
    ...
    if asOx then
        registerOxLibMenu(mainMenuID, mainMenu.title, menuItems, nil)
        lib.showMenu(mainMenuID)
    else
    ...
    lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
        registerOutfitMenus(outfits or {})
    end)
end
